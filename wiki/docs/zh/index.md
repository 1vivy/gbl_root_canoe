...
features:
  - title: 一个 Canoe Boot Manager
    details: 一个 Svelte 应用与一套部署引擎，同时服务于在线浏览器运行时与 KernelSU Android WebUI
    link: /zh/intro
  - title: 在线浏览器应用
    details: 从 Overview 开始，然后使用 Deploy 的 Provision、Prepare、Review 阶段以及 Entries、Settings、Diagnostics
    link: /zh/usage
  - title: KernelSU Android
    details: 同一应用直接读取本地启动根目录；Overview 报告状态，Deploy 提供相应 lane
    link: /zh/usage
  - title: CLI
    details: canoe-bootmgr 写入已挂载的启动根目录，canoe-image 准备镜像，canoe-provision 管理容器
    link: /zh/usage
  ...
  - title: BDS 配置与链式启动
    ...
    link: /zh/canoe-cfg
  - title: 构建
    details: 从源码构建 CANOE-BDS、独立 EFI 工具与 KernelSU 模块
    link: /zh/build
  - title: 贡献
