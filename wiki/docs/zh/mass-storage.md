...
**导出的身份。** 每次开机后的第一个会话开始之前，BDS 会在内存中改写驻留
导出驱动的呈现：设备枚举为 **`1209:ca0e`**（产品名 `efisp boot root`，固定
磁盘），而不是高通的 `05c6:f000`。没有任何系统规则认领这一身份，因此打过
补丁的导出不会再遇到下面的 modeswitch 问题；当固件布局使补丁无法定位时仍
回退为 `05c6:f000`，两种身份工具都会识别。菜单中新增的第四行 **msdimage**
（以及 `fastboot oem mass-storage:msdimage`）会把驻留驱动本身作为 RAM 磁盘
导出，各固件的校准数据正是经由此进入
...
`canoe install` 通过 USB 身份（`1209:ca0e`，未打补丁时为 `05c6:f000`）识别
LUN，而不是等待新的磁盘
