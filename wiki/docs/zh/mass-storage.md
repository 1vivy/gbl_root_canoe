...
**导出的身份。** BDS 自带平台大容量存储驱动的一个变体（离线打好补丁，按需
启动）：设备枚举为 **`1209:ca0e`**（产品名 `efisp boot root`，固定磁盘），
而不是高通的 `05c6:f000`。没有任何系统规则认领这一身份，因此变体导出不会
再遇到下面的 modeswitch 问题；当变体无法启动时由驻留驱动以原始呈现接管，
两种身份工具都会识别。菜单中新增的第四行 **msdimage**（以及
`fastboot oem mass-storage:msdimage`）会把驻留驱动本身作为 RAM 磁盘导出，
各固件的校准数据正是经由此进入
