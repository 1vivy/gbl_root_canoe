...
/*
 * There is deliberately no efisp Block I/O hook here. The ABL patcher owns
 * recursion prevention; hiding efisp from UEFI would only matter if we
 * deliberately ran the older on-slot ABL at runtime, which we do not do, and
 * an unpopulated boot root is recovered through the documented Revert path
 * (flash the source ABL, wipe efisp) from Super Fastboot.
 */
