/** @file
 *  Write the bounded passive SurfaceTools inventory to logfs.
 *
  ...
  )
{
  ...
  *File = NULL;
  Status = Root->Open (Root, File, ST_DUMP_PATH,
  ...
  return Root->Open (Root, File, ST_DUMP_PATH,
...
StDumpPassiveInventory (VOID)
{
  ...
  Status = StCreateDumpFile (Root, &File);
