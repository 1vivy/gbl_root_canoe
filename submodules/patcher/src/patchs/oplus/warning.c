...
bool patch_warning(char* buffer, int32_t size, int32_t global_var_offset) {
    ...
    if (warn_off < 0) {
        PATCH_LOG("Warning not found\n");
        return false;
    }
    PATCH_LOG("Warning message ADRL offset: 0x%X\n", warn_off);
    //逆向找到CBZ
    ...
    for(int32_t i = warn_off - 4; i >= max_search_range; i -= 4) {
        ...
        if (d.type == INST_PACIASP) {
            PATCH_LOG("Reached function start at 0x%X, stop\n", i);
            return false;
        }
        if (d.type == INST_CBZ_W) {
            PATCH_LOG("Register W%d is used for warning jump at 0x%X\n", d.rt, i);
            //track back
            int32_t offset = -1;
            ...
            PATCH_LOG("Warning jump source var: 0x%X Matched\n", offset);
            write_instr(buffer,i,change_rt(&d, 31)); // change to CBZ WZR
            PATCH_LOG("Patched CBZ at 0x%X to use WZR, warning disabled\n", i);
            break;
        }
    }
    return true;
