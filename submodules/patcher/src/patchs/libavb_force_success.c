...
static const uint8_t Patch10Anchor[] =
...
static PATCH10_RESULT FindUniqueAnchor(const uint8_t *Buffer,
                                       size_t Size,
                                       size_t *Offset) {
    size_t AnchorSize = sizeof(Patch10Anchor) - 1;
    size_t Found = 0;
    size_t First = 0;
    size_t Index;

    if (Offset == NULL || AnchorSize == 0 || Size < AnchorSize) {
        return PATCH10_FAILURE;
    }
    for (Index = 0; Index <= Size - AnchorSize; ++Index) {
        if (memcmp(Buffer + Index, Patch10Anchor, AnchorSize) == 0) {
            if (Found == 0) {
                First = Index;
            }
            ++Found;
    ...
    if (Found != 1) {
        return PATCH10_AMBIGUOUS;
    }
    *Offset = First;
    return PATCH10_SUCCESS;
}

static PATCH10_RESULT FindAdrpAddReference(const PE_IMAGE *Image,
                                           uint32_t TargetRva,
                                           size_t *Offset,
                                           size_t *SectionIndex) {
    size_t Found = 0;
...
PATCH10_RESULT patch_libavb_force_success(char *Buffer, int32_t Size) {
    ...
    if (!PeImageFileOffsetToRva(&Image, AnchorOffset,
                                sizeof(Patch10Anchor) - 1, &AnchorRva)) {
        return PATCH10_FAILURE;
    }
