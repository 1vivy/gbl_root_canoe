...
use crate::build_tools::{self, ToolOutput, ToolResolver, WorkDir};
...
pub(crate) fn patch_loader(
    ...
    let output = run_step(
        &tools.resolve("patch_abl")?,
        vec![arg(loader), arg(boot)],
        "patch_abl",
    )?;
