...
pub fn create_staging(path: &Path) -> io::Result<VolumeInfo> {
    let mut file = OpenOptions::new()
        ...
        .create_new(true)
        .open(path)?;
    // set_len/fallocate alone can produce holes or unwritten extents. Write
    // every byte, including the unused FAT data area, before the installer
    // checks the extent map and commits the final name.
    let zeros = [0u8; 64 * 1024];
    for _ in 0..CONTAINER_BYTES / zeros.len() as u64 {
