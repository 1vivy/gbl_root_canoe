...
class CiScopeTests(unittest.TestCase):
    def test_paths_select_owning_checks_without_unrelated_firmware_builds(self):
        cases = [
            ('docs/usage.md', set(), set()),
            ('submodules/uefi/tests/test_launch.c', {'uefi_tests'}, set()),
            ('submodules/uefi/edk2/QcomModulePkg/Application/LinuxLoader/TestBootRoot.c', {'uefi_tests'}, set()),
            ('submodules/uefi/edk2/QcomModulePkg/Application/LinuxLoader/SuperFbMenu.c', {'uefi_tests', 'firmware', 'versions'}, set()),
            ('tools/canoe-fs/tests/files.rs', set(), {'canoe-fs'}),
            ('tools/canoe-fs/src/lib.rs', set(), {'canoe-fs', 'canoe-image', 'canoe-bootmgr', 'mode2-profile'}),
            ('tools/canoe-image/tests/android.rs', set(), {'canoe-image'}),
            ('submodules/patcher/tests/test_libavb_force_success.c', {'patcher_tests'}, set()),
            ('submodules/patcher/src/patchs/core.c', {'patcher_tests'}, {'canoe-image'}),
            ('targets/magisk_module/module/customize.sh', {'module_tests'}, set()),
            ('scripts/tests/test_imports_check.py', {'script_tests'}, set()),
            ('scripts/firmware_release.py', {'script_tests', 'firmware', 'versions'}, set()),
        ]
        for path, expected_duties, expected_crates in cases:
