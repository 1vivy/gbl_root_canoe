...
echo "     ./canoe_prep_device.sh    # derive from the device's own abl/vbmeta"
echo "     ./canoe_stage.sh          # install the persist tree, then the BDS"
echo "Alongside a Super Flasher / RegionalHybrid package:"
echo "     ./canoe_prep.sh --pkg <dir> --recovery <custom>.img \\"
echo "                     --abl <vulnerable>.img --in-place"
echo "     # run the package's own flasher, then:"
echo "     ./canoe_stage.sh"
...
echo "   (from a custom recovery the boot root is /persist/efisp instead;"
echo "    create the directory first if needed, e.g. via MT Manager)"
echo "2. sync"
if [ "$gbl_ok" = "no" ]; then
