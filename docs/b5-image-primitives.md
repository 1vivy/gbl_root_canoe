...
- vendor_boot patching contains a largely pure header/CPIO/gzip/legacy-LZ4 pipeline,
  currently exported through a filesystem API. LZ4's C dependency alone is not
  evidence of incompatibility: an isolated WASM target check passes. Actual linked
  module behavior must be checked before choosing a replacement or exporting it.
